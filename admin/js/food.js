document.addEventListener("DOMContentLoaded", async function () {
     ...
     if (!Number.isInteger(Number(price))) {
      alertMessage("Price must be a valid integer.");
        ...
        if (!name || !price || !category || !available || !image) {
          ...
          formData.append("food_beverage", food_beverage); 
          formData.append("description", description); 
  
          
          const response = await fetch("http://localhost/TheGalleryCafe/controller/addFood.php", {
              method: "POST",
              body: formData
          });
  ...
  async function editFood(id) {
    const response = await fetch(`http://localhost/TheGalleryCafe/controller/getFoodById.php?id=${id}`);
    const data = await response.json();
    ...
    document.getElementById("update-food_beverage").value = data.food_or_beverage;
...
document.getElementById("form-food-update").addEventListener("submit", async function (e) {
  e.preventDefault();
  ...
  const available = document.querySelector('input[name="update-available"]:checked');
  ...
  const food_beverage = document.getElementById("update-food_beverage").value.trim();
  ...
  if (!Number.isInteger(Number(price))) {
      ...
      if (!name || !price || !category || !available) {
          ...
          const response = await fetch("http://localhost/TheGalleryCafe/controller/updateFood.php", {
              method: "POST",
              body: formData
          });
